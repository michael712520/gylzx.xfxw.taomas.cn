...
function insert_supplier_list($arr){
	...
	if(empty($tag_ret)){//没有标签
		$sql = "SELECT ssc.value,ssc.supplier_id,ssc.code FROM ".$GLOBALS['ecs']->table('supplier_street')." as ss,".$GLOBALS['ecs']->table('supplier_shop_config')." as ssc WHERE ss.`status` = 1 AND ssc.supplier_id = ss.supplier_id AND code in('shop_name','shop_logo') order by ss.sort_order limit 36";
		...
		$sql = " select ssc.value,ssc.supplier_id,ssc.code from ".$GLOBALS['ecs']->table('supplier_tag_map')." as stm,".
			   ...
			   " where stm.tag_id=".$tag_id." AND ssc.supplier_id = stm.supplier_id AND ss.supplier_id = stm.supplier_id AND code in('shop_name','shop_logo') order by ss.sort_order limit 36";
	}
