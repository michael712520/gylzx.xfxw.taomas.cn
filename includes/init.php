...
if(empty($_SESSION['user_id']) 
		&& !strstr($_SERVER['PHP_SELF'],"user.php")
		&& !strstr($_SERVER['PHP_SELF'],"captcha.php")
		&& !strstr($_SERVER['PHP_SELF'],"apply_index.php")
        && !strstr($_SERVER['PHP_SELF'],"register.php")
	)
{
	
	show_message('未登录无法浏览页面', array(
			'</br>登录'
	), array(
			'user.php?act=login'), 'error', true);
