...
$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";
    $sql = "SELECT c.rec_id, c.user_id, c.goods_id, c.goods_name, c.goods_sn, c.goods_number, c.market_price, " .
			...
			" ssc.value as shop_monetary_limitation ".
			...
			" left join " . $GLOBALS['ecs']->table('supplier_shop_config') . " as ssc on ssc.supplier_id = ga.supplier_id and ssc.code='shop_monetary_limitation'" .
			" WHERE $sql_where " .
...
function get_cart_goods($other='')
{
    ...
    /* 代码增加_start    By   www.68ecshop.com */
	$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";
	$sql = "SELECT g.buy_num_min, c.*,g.buy_num_min, g.cat_id, g.brand_id, IF(ga.act_id, ga.supplier_id, g.supplier_id) as supplier_id, IF(c.parent_id, c.parent_id, c.goods_id) AS pid  ,ssa.value as shop_monetary_limitation" .
            " FROM " . $GLOBALS['ecs']->table('cart') . " AS c left join " .$GLOBALS['ecs']->table('goods')." AS g ".
			" on c.goods_id=g.goods_id ".
			" left join " . $GLOBALS['ecs']->table('goods_activity') . " as ga " .
			" on ga.act_id = c.goods_id and c.extension_code = 'package_buy'" .
			" left join " . $GLOBALS['ecs']->table('supplier_shop_config') . " as ssa ".
			" on ssa.supplier_id = IF(ga.act_id, ga.supplier_id, g.supplier_id) and ssa.code = 'shop_monetary_limitation'" .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if(empty($supplier_shop_total[$row['supplier_id']])){
        	$supplier_shop_total[$row['supplier_id']]['supplier_shop_total']=$row['goods_price'] * $row['goods_number'];
        	$supplier_shop_total[$row['supplier_id']]['shop_monetary_limitation']=$row['shop_monetary_limitation'];
        }else{
        	$supplier_shop_total[$row['supplier_id']]['supplier_shop_total']+=$row['goods_price'] * $row['goods_number'];
        	$supplier_shop_total[$row['supplier_id']]['shop_monetary_limitation']=$row['shop_monetary_limitation'];
        }
        ...
        $row['supplier_shop_total']+=price_format($row['goods_price'] * $row['goods_number'], false);
...
/* 代码增加_start  By  www.68ecshop.com */
    ...
    return array('goods_list' => $goods_list, 'total' => $total,'supplier_shop_total'=>$supplier_shop_total);
}
