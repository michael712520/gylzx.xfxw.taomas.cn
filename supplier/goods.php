...
elseif ($_REQUEST ['act'] == 'add' || $_REQUEST ['act'] == 'edit' || $_REQUEST ['act'] == 'copy') {
	...
	/* 模板赋值 */
	$action_link_supplier = $is_add ? array (
			'href' => 'goods.php?act=list&supplier_status=0',
			'text' => '返回商品列表' 
	) : array (
			'href' => 'goods.php?act=list&supplier_status=' . $_REQUEST ['supplier_status'],
			'text' => '返回商品列表' 
	);
	$smarty->assign ( 'code', $code );
	...
	if (isset ( $_FILES ['goods_img'] ['error'] )) // php 4.2 版本才支持 error
{
	...
	if ($is_insert) {
		if ($code == '') {
			$sql = "INSERT INTO " . $ecs->table ( 'goods' ) . " (goods_name, goods_name_style, goods_sn, " . "cat_id, brand_id, shop_price, market_price, is_promote, zhekou, promote_price, " . "promote_start_date, promote_end_date, is_buy,buymax,buymax_start_date,buymax_end_date,goods_img, goods_thumb, original_img, keywords, goods_brief, " . "seller_note, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot, " . "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral,exclusive, supplier_id,supplier_status)" . "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " . "'$brand_id', '$shop_price', '$market_price', '$is_promote', '$zhekou', '$promote_price', " . "'$promote_start_date', '$promote_end_date', '$is_buy','$buymax','$buymax_start_date','$buymax_end_date','$goods_img', '$goods_thumb', '$original_img', " . "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number'," . " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', '0', '$is_alone_sale', $is_shipping, " . " '$_POST[goods_desc]', '" . gmtime () . "', '" . gmtime () . "', '$goods_type', '$rank_integral','$exclusive', '$supplier_id', '0')";
		} else {
			$sql = "INSERT INTO " . $ecs->table ( 'goods' ) . " (goods_name, goods_name_style, goods_sn, " . "cat_id, brand_id, shop_price, market_price, is_promote, zhekou, promote_price, " . "promote_start_date, promote_end_date, is_buy,buymax,buymax_start_date,buymax_end_date,goods_img, goods_thumb, original_img, keywords, goods_brief, " . "seller_note, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot, is_real, " . "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code,exclusive, rank_integral, supplier_status)" . "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " . "'$brand_id', '$shop_price', '$market_price', '$is_promote', '$zhekou', '$promote_price', " . "'$promote_start_date', '$promote_end_date', '$is_buy','$buymax','$buymax_start_date','$buymax_end_date','$goods_img', '$goods_thumb', '$original_img', " . "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number'," . " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, " . " '$_POST[goods_desc]', '" . gmtime () . "', '" . gmtime () . "', '$goods_type', '$code','$exclusive', '$rank_integral','0')";
		}
	} else {
		/* 如果有上传图片，删除原来的商品图 */
		$sql = "SELECT goods_thumb, goods_img, original_img, supplier_status " . " FROM " . $ecs->table ( 'goods' ) . " WHERE goods_id = '$_REQUEST[goods_id]'";
		$row = $db->getRow ( $sql );
		...
		$sql = "UPDATE " . $ecs->table ( 'goods' ) . " SET " . "goods_name = '$_POST[goods_name]', " . "goods_name_style = '$goods_name_style', " . "goods_sn = '$goods_sn', " . "cat_id = '$catgory_id', " . "brand_id = '$brand_id', " . "shop_price = '$shop_price', " . "exclusive = '$exclusive', " . // 手机专享价 app jx
"market_price = '$market_price', " . "is_promote = '$is_promote', " . "zhekou = '$zhekou', " . "promote_price = '$promote_price', " . "promote_start_date = '$promote_start_date', " . "is_buy = '$is_buy', " . "buymax = '$buymax', " . "buymax_start_date = '$buymax_start_date', " . "buymax_end_date = '$buymax_end_date', " . "supplier_id = '$supplier_id', " . "promote_end_date = '$promote_end_date', ";
		...
		if (isset ( $_POST ['txm_shu'] ) && isset ( $_POST ['tiaoxingm'] ) || (empty ( $_POST ['txm_shu'] )) && (empty ( $_POST ['tiaoxingm'] ))) {
			...
			foreach ( $type as $key => $value ) {
				foreach ( $bar_code as $k => $v ) {
					...
					if ($key == $k) {
						$sql = "INSERT INTO " . $ecs->table ( 'bar_code' ) . " (goods_id, taypes, bar_code) " . "VALUES ('$arr[goods_id]', '$arr[taypes]','$arr[bar_code]')"; // 插入数据
						$name = $db->query ( $sql );
					}
				}
			}
	...
	if (check_goods_specifications_exist ( $goods_id )) {
		$link [0] = array (
				'href' => 'goods.php?act=product_list&supplier_status=' . $_REQUEST ['supplier_status'] . '&goods_id=' . $goods_id,
				'text' => $_LANG ['product'] 
		);
	}
	if ($code == 'virtual_card') {
...
elseif ($_REQUEST ['act'] == 'batch') {
	...
	if ($_POST ['type'] == 'drop' || $_POST ['type'] == 'restore') {
		$link [] = array (
				'href' => 'goods.php?act=trash',
				'text' => $_LANG ['11_goods_trash'] 
		);
	} else {
...
"g.sort_order, " . // 推荐排序
"g.goods_number " . // 库存
"FROM " . $GLOBALS ['ecs']->table ( 'goods' ) . 'AS g ' . "LEFT JOIN " . $GLOBALS ['ecs']->table ( 'supplier_goods_cat' ) . " AS sg " . "ON g.goods_id = sg.goods_id " . $where;
	...
	foreach ( $res as $value ) {
		// 设置所有垂直居中
		$objPHPExcel->getActiveSheet ()->getStyle ( 'A' . $row_num . ':' . 'J' . $row_num )->getAlignment ()->setVertical ( PHPExcel_Style_Alignment::VERTICAL_CENTER );
		// 设置价格为数字格式
		$objPHPExcel->getActiveSheet ()->getStyle ( 'D' . $row_num )->getNumberFormat ()->setFormatCode ( PHPExcel_Style_NumberFormat::FORMAT_NUMBER_00 );
		// 居中
		$objPHPExcel->getActiveSheet ()->getStyle ( 'E' . $row_num . ':' . 'H' . $row_num )->getAlignment ()->setHorizontal ( PHPExcel_Style_Alignment::HORIZONTAL_CENTER );
...
elseif ($_REQUEST ['act'] == 'drop_goods') {
	...
	/* 取得商品信息 */
	$sql = "SELECT goods_id, goods_name, is_delete, is_real, goods_thumb, " . "goods_img, original_img ,supplier_id" . " FROM " . $ecs->table ( 'goods' ) . " WHERE goods_id = '$goods_id'";
	$goods = $db->getRow ( $sql );
...
elseif ($_REQUEST ['act'] == 'product_list') {
	...
	if (empty ( $attribute )) {
		$link [] = array (
				'href' => 'goods.php?act=edit&goods_id=' . $goods_id,
				'text' => $_LANG ['edit_goods'] 
		);
...
elseif ($_REQUEST ['act'] == 'product_query') {
	...
	make_json_result ( $smarty->fetch ( 'product_info.htm' ), '', array (
...
elseif ($_REQUEST ['act'] == 'product_add_execute') {
	...
	if ($insert) {
		...
		$link [] = array (
				'href' => 'goods.php?act=product_list&supplier_status=' . $_REQUEST ['supplier_status'] . '&goods_id=' . $product ['goods_id'],
				'text' => $_LANG ['18_product_list'] 
		);
	} else {
		$link [] = array (
				'href' => 'goods.php?act=list&supplier_status=' . $_REQUEST ['supplier_status'] . '&uselastfilter=1',
				'text' => $_LANG ['01_goods_list'] 
		);
		$link [] = array (
				'href' => 'goods.php?act=edit&goods_id=' . $product ['goods_id'],
				'text' => $_LANG ['edit_goods'] 
		);
		$link [] = array (
				'href' => 'goods.php?act=product_list&supplier_status=' . $_REQUEST ['supplier_status'] . '&goods_id=' . $product ['goods_id'],
...
elseif ($_REQUEST ['act'] == 'batch_product') {
	/* 定义返回 */
	$link [] = array (
			'href' => 'goods.php?act=product_list&goods_id=' . $_POST ['goods_id'],
			'text' => $_LANG ['item_list'] 
	);
...
function handle_other_cat2($goods_id, $cat_list) {
	/* 查询现有的扩展分类 */
	$sql = "SELECT cat_id FROM " . $GLOBALS ['ecs']->table ( 'supplier_goods_cat' ) . " WHERE goods_id = '$goods_id' and supplier_id= '$_SESSION[supplier_id]' ";
	$exist_list = $GLOBALS ['db']->getCol ( $sql );
	
	/* 删除不再有的分类 */
	$delete_list = array_diff ( $exist_list, $cat_list );
	if ($delete_list) {
		$sql = "DELETE FROM " . $GLOBALS ['ecs']->table ( 'supplier_goods_cat' ) . " WHERE goods_id = '$goods_id' AND supplier_id= '$_SESSION[supplier_id]' " . "AND cat_id " . db_create_in ( $delete_list );
		$GLOBALS ['db']->query ( $sql );
	}
	...
	foreach ( $add_list as $cat_id ) {
		// 插入记录
		$sql = "INSERT INTO " . $GLOBALS ['ecs']->table ( 'supplier_goods_cat' ) . " (goods_id, cat_id, supplier_id) " . "VALUES ('$goods_id', '$cat_id', '$_SESSION[supplier_id]')";
		$GLOBALS ['db']->query ( $sql );
	}
}
