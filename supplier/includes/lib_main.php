...
function get_bonus_type() {
	$bonus = array ();
	$sql = 'SELECT type_id, type_name, type_money FROM ' . $GLOBALS ['ecs']->table ( 'bonus_type' ) . ' WHERE send_type = 3 AND supplier_id=' . $_SESSION ['supplier_id'];
...
function get_position_list() {
	$position_list = array ();
	$sql = 'SELECT position_id, position_name, ad_width, ad_height ' . 'FROM ' . $GLOBALS ['ecs']->table ( 'ad_position' );
	$res = $GLOBALS ['db']->query ( $sql );
...
function cat_exists_supplier($suppid, $cat_name, $parent_cat, $exclude = 0) {
	$sql = "SELECT COUNT(*) FROM " . $GLOBALS ['ecs']->table ( 'supplier_category' ) . " WHERE supplier_id=" . $suppid . " AND parent_id = '$parent_cat' AND cat_name = '$cat_name' AND cat_id<>'$exclude'";
...
function cat_list_2($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true) {
	...
	if ($res === NULL) {
		...
		$sql = "SELECT gc.cat_id, COUNT(*) AS goods_num " . " FROM " . $GLOBALS ['ecs']->table ( 'supplier_goods_cat' ) . " AS gc , " . $GLOBALS ['ecs']->table ( 'goods' ) . " AS g " . " WHERE gc.supplier_id = " . $_SESSION ['supplier_id'] . " AND g.goods_id = gc.goods_id AND g.is_delete = 0 AND g.is_on_sale = 1 " . " GROUP BY gc.cat_id";
	...
	if ($re_type == true) {
		...
		foreach ( $options as $key => $value ) {
			$options [$key] ['url'] = build_uri ( 'supplier', array (
					'go' => 'category',
...
function create_shop_settiongs() {
	...
	if ($num > 0) {
		...
		$insql = "INSERT INTO " . $ecs->table ( 'supplier_shop_config' ) . " (`id`, `parent_id`, `code`, `type`, `store_range`, `store_dir`, `value`, `sort_order`, `supplier_id`) VALUES
				...
				(126, 1, 'shop_monetary_limitation', 'text', '', '', '店铺最低金额', 1, " . $_SESSION ['supplier_id'] . "),
