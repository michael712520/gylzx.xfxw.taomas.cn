...
if (! empty ( $_REQUEST ['act'] ) && $_REQUEST ['act'] == 'selcart') {
	...
	if ($_GET ['sel_goods']) {
		...
		$res['supplier_shop_total']=json_encode($cart_goods['supplier_shop_total']);
		// 折扣活动
...
elseif ($_REQUEST ['act'] == 'selAddress') {
	...
	$db->query ( $sql );
	$sql = "SELECT * " . " FROM " . $GLOBALS ['ecs']->table ( 'user_address' ) . " WHERE  address_id = '" . $_REQUEST ['address_id'] . "' ";
...
elseif ($_REQUEST ['act'] == 'delAddress') {
	...
	if ($_SESSION ['flow_consignee'] ['address_id'] == $address_id) {
		if ($consignee_list) {
			...
			$region = array (
					$consignee_list [0] ['country'],
					$consignee_list [0] ['province'],
					$consignee_list [0] ['city'],
					$consignee_list [0] ['district'] 
			);
		} else {
...
elseif ($_REQUEST ['act'] == 'saveAddress') {
	...
	$result = array (
			...
			'message' => '',
			'content' => '',
			'closediv' => $address_ecshop68->closediv 
	);
	$consignee_list = get_consignee_list_ecshop68 ();
	$smarty->assign ( 'consignee_list', $consignee_list );
	$result ['content'] = $smarty->fetch ( "library/address_list.lbi" );
	...
	if ($address_ecshop68->shipping_bian == '0' && $address_ecshop68->address_id > 0 && $address_ecshop68->address_id == $_SESSION ['flow_consignee'] ['address_id']) {
		$sql = "SELECT * " . " FROM " . $GLOBALS ['ecs']->table ( 'user_address' ) . " WHERE  address_id = '" . $address_ecshop68->address_id . "' ";
		$consignee = $GLOBALS ['db']->getRow ( $sql );
		$_SESSION ['flow_consignee'] = $consignee;
	}
...
if ($_REQUEST ['step'] == 'add_to_cart') {
	...
	$sql = 'SELECT SUM(goods_number) AS number, SUM(goods_price * goods_number) AS amount' . ' FROM ' . $GLOBALS ['ecs']->table ( 'cart' ) . " WHERE " . $sql_where . " AND rec_type = '" . CART_GENERAL_GOODS . "'";
...
elseif ($_REQUEST ['step'] == 'link_buy') {
	...
	if ($_SERVER ['REQUEST_METHOD'] == 'GET') {
		...
		$sql = "SELECT COUNT(*) FROM " . $ecs->table ( 'cart' ) . " WHERE $sql_where AND is_gift > 0";
		if ($db->getOne ( $sql ) > 0) {
...
elseif ($flow_type == CART_EXCHANGE_GOODS) {
	...
	if ($flow_type != CART_EXCHANGE_GOODS) {
		...
		if ($favourable_list) {
			...
			foreach ( $favourable_list as $fk => $fv ) {
				if (! $fv ['available']) {
					$sql = "select count(rec_id) as num from " . $ecs->table ( 'cart' ) . " WHERE $sql_where " . "AND is_gift = " . $fv ['act_id'] . $sql_plus;
					if ($db->getOne ( $sql ) > 0) {
	...
	$sql = "SELECT COUNT(*) FROM " . $ecs->table ( 'cart' ) . " WHERE $sql_where " . "AND parent_id = 0 AND is_gift = 0 AND rec_type = '$flow_type'";
	/* 代码增加_end By www.68ecshop.com */
	if ($db->getOne ( $sql ) == 0) {
...
// 			echo var_dump ( $val ['supplier_id'] ['goodlist'] [0] ['shop_monetary_limitation'] );
// 			$cart_goods_new [$val ['supplier_id']] ['shop_monetary_limitation'] [] = $val ['supplier_id'] ['goodlist'] [0] ['shop_monetary_limitation'];
			$cart_goods_new [$val ['supplier_id']] ['shipping_html'] = insert_get_shop_shipping ( array (
					'suppid' => $val ['supplier_id'],
					'consignee' => $consignee,
					'flow_type' => $flow_type 
			) );
		}
	...
	if ($consignee ['city']) {
		$sql = 'select r.region_name, r.region_id from ' . $ecs->table ( 'pickup_point' ) . ' p left join ' . $ecs->table ( 'region' ) . ' r on p.city_id=r.region_id where p.city_id=' . $consignee ['city'];
	...
	if ((! isset ( $_CFG ['use_bonus'] ) || $_CFG ['use_bonus'] == '1') && ($flow_type != CART_GROUP_BUY_GOODS && $flow_type != CART_EXCHANGE_GOODS && $flow_type != CART_PRE_SALE_GOODS)) {
		// 取得用户可用红包
...
elseif ($_REQUEST ['step'] == 'done') {
	...
	// 获取余额支付的id
	$sql = 'SELECT pay_id ' . ' FROM ' . $GLOBALS ['ecs']->table ( 'payment' ) . ' WHERE enabled = 1 and pay_code="balance"';
	...
	foreach ( $cart_goods_new as $ckey => $cval ) {
		...
		if ($_REQUEST ['inv_type'] == 'normal_invoice') {
			...
			$inv_arr = array (
					...
					'inv_consignee_name',
					'inv_consignee_phone',
					'inv_consignee_province',
					'inv_consignee_city',
					'inv_consignee_district',
			...
			if ($order ['surplus'] > 0) // 余额支付里如果输入了一个金额
{
	...
	foreach ( $order_info as $ok => $order ) {
		...
		/* 代码增加_start By www.68ecshop.com */
		$sql = "INSERT INTO " . $ecs->table ( 'order_goods' ) . "( " . "order_id, goods_id, goods_name, goods_sn, product_id, goods_number, market_price, cost_price, " . "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id, package_attr_id) " . " SELECT '$new_order_id', goods_id, goods_name, goods_sn, product_id, goods_number, market_price, cost_price, " . "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id, package_attr_id " . " FROM " . $ecs->table ( 'cart' ) . " WHERE $sql_where AND rec_type = '$flow_type' $id_ext_new ";
		/* 代码增加_end By www.68ecshop.com */
		$db->query ( $sql );
		/* 修改拍卖活动状态 */
		...
		if ($order ['order_amount'] <= 0) {
			/* 代码增加_start By www.68ecshop.com */
			$sql = "SELECT goods_id, goods_name,extension_code, goods_attr_id, goods_number AS num FROM " . $GLOBALS ['ecs']->table ( 'cart' ) . " WHERE is_real = 0 " . " AND $sql_where AND rec_type = '$flow_type'";
			/* 代码增加_end By www.68ecshop.com */
			$res = $GLOBALS ['db']->getAll ( $sql );
...
elseif ($_REQUEST ['step'] == 'add_favourable') {
	...
	if ($favourable ['act_type'] == FAT_GOODS) {
		...
		$sql_where = $_SESSION ['user_id'] > 0 ? "user_id='" . $_SESSION ['user_id'] . "' " : "session_id = '" . SESS_ID . "' AND user_id=0 "; // 添加 www.68ecshop.com
		/* 检查是否已在购物车 */
		$sql = "SELECT goods_name" . " FROM " . $ecs->table ( 'cart' ) . " WHERE $sql_where " . " AND rec_type = '" . CART_GENERAL_GOODS . "'" . " AND is_gift = '$act_id'" . " AND goods_id " . db_create_in ( $_POST ['gift'] );
...
elseif ($_REQUEST ['step'] == 'add_package_to_cart') {
	...
	if ($_COOKIE ['one_step_buy'] == '1') {
		$sql_where = $_SESSION ['user_id'] > 0 ? "user_id='" . $_SESSION ['user_id'] . "' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";
		$sql = "SELECT rec_id FROM " . $ecs->table ( 'cart' ) . " WHERE $sql_where AND rec_type = '" . $flow_type . "'";
	...
	// 购物车的描述的格式化
	$smarty->assign ( 'supplier_shop_total', json_encode($cart_goods ['supplier_shop_total'] ));
...
if ($_REQUEST ['step'] == 'update_group_cart') {
	...
	if ($GLOBALS ['_CFG'] ['use_storage'] == 1) {
		...
		if ($is_package == 0) {
			...
			$buy_num_min = $GLOBALS ['db']->getOne ( "select buy_num_min from " . $GLOBALS ['ecs']->table ( 'goods' ) . " where goods_id='$goods_id'" );
			...
			if ($number2 > $goods_number) // ////// jx 库存判断
			{
				...
				if($goods_number<=$buy_num_min){
					$result ['error'] = '1';
					$result ['content'] = '对不起,您选择的数量低于起订数量' . $buy_num_min . "件";
					$result ['content'] = "对不起,您选择的数量低于起订数量" . $buy_num_min . "件";
					if ($pg_num > 0) {
						$result ['content'] .= ",礼包中已包含此单品 " . $pg_num . "件";
					}
					$result ['number'] = $GLOBALS ['db']->getOne ( "select buy_num_min from " . $GLOBALS ['ecs']->table ( 'cart' ) . " where rec_id = '$rec_id'" );
					if(intval($result ['number'])==0){
						$result ['number']=1;
					}
					die ( $json->encode ( $result ) );
				}
				...
				if ($number2 > $goods_number) // ////// jx 库存判断
{
	...
	if ($_CFG ['show_marketprice']) {
		$market_price_desc = sprintf ( $_LANG ['than_market_price'], $cart_goods ['total'] ['market_price'], $cart_goods ['total'] ['saving'], $cart_goods ['total'] ['save_rate'] );
		$result['supplier_shop_total']=json_encode($cart_goods['supplier_shop_total']);
...
function cart_goods_recommend($rtype) {
	...
	while ( $row_hot = $GLOBALS ['db']->fetchRow ( $res_hot ) ) {
		...
		$row_hot ['url'] = build_uri ( 'goods', array (
				'gid' => $row_hot ['goods_id'] 
		), $row_hot ['goods_name'] );
...
// 线上红包
$sql = "SELECT * FROM " . $GLOBALS ['ecs']->table ( 'bonus_type' ) . " WHERE send_type = '4'";
	...
	foreach ( $arr as $key => $val ) {
		...
		if ($val > 0) {
			...
			if ($goods ['extension_code'] == 'package_buy') {
				// 更新购物车中的商品数量
				$sql = "UPDATE " . $GLOBALS ['ecs']->table ( 'cart' ) . " SET goods_number = '$val' WHERE rec_id='$key' AND $sql_where1";
			}  /* 处理普通商品或非优惠的配件 */
else {
				...
				// 更新购物车中的商品数量
				/* 代码修改_start By www.68ecshop.com 将这块替换掉 */
				$sql = "UPDATE " . $GLOBALS ['ecs']->table ( 'cart' ) . " SET goods_number = '$val', goods_price = '$goods_price' WHERE rec_id='$key' AND $sql_where1";
				/* 代码修改_end By www.68ecshop.com */
			}
		}  // 订货数量等于0
else {
			...
			/* 代码修改_start By www.68ecshop.com 将这块替换掉 */
			while ( $offers_accessories_row = $GLOBALS ['db']->fetchRow ( $offers_accessories_res ) ) {
				$sql = "DELETE FROM " . $GLOBALS ['ecs']->table ( 'cart' ) . " WHERE $sql_where1 " . "AND rec_id = '" . $offers_accessories_row ['rec_id'] . "' LIMIT 1";
				$GLOBALS ['db']->query ( $sql );
			}
			
			$sql = "DELETE FROM " . $GLOBALS ['ecs']->table ( 'cart' ) . " WHERE rec_id='$key' AND $sql_where1";
			/* 代码修改_end By www.68ecshop.com */
		}
...
function flow_cart_stock($arr) {
	foreach ( $arr as $key => $val ) {
		...
		/* 代码修改_start By www.68ecshop.com 将这块替换掉 */
		$sql_where = $_SESSION ['user_id'] > 0 ? "user_id='" . $_SESSION ['user_id'] . "' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";
		$sql = "SELECT `goods_id`, `goods_attr_id`, `extension_code` FROM" . $GLOBALS ['ecs']->table ( 'cart' ) . " WHERE rec_id='$key' AND $sql_where";
		$goods = $GLOBALS ['db']->getRow ( $sql );
		/* 代码修改_end By www.68ecshop.com */
...
function flow_drop_cart_goods($id) {
	...
	if ($row) {
		...
		elseif ($row ['parent_id'] == 0 && $row ['is_gift'] == 0) {
			...
			$sql = "DELETE FROM " . $GLOBALS ['ecs']->table ( 'cart' ) . " WHERE $sql_where " . "AND ( $sql_plus  parent_id = '$row[goods_id]' OR is_gift <> 0)";
			/* 代码修改_end By www.68ecshop.com */
		} 		
...
function cart_favourable() {
	...
	$list = array ();
	/* 代码修改_start By www.68ecshop.com 将这块替换掉 */
	$sql = "SELECT is_gift, COUNT(*) AS num " . "FROM " . $GLOBALS ['ecs']->table ( 'cart' ) . " WHERE $sql_where " . " AND rec_type = '" . CART_GENERAL_GOODS . "'" . " AND is_gift > 0" . " GROUP BY is_gift";
	/* 代码修改_end By www.68ecshop.com */
	$res = $GLOBALS ['db']->query ( $sql );
	while ( $row = $GLOBALS ['db']->fetchRow ( $res ) ) {
